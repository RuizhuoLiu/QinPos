...
def _explode(row) -> list[tuple]:
    ...
    n = max(map(len, (degrees, ranges, strings, positions, timbres)))
    ...
    multi = n > 1
    return [
        (int(d), int(r), int(s), p, None if t is None else int(t), multi)
...
def load_piece(path: Path) -> list[Event]:
    ...
    for sheet_name, df in pd.read_excel(path, sheet_name=None).items():
        ...
        for _, row in rows.iterrows():
            ...
            for deg, rng, st, pos, tim, multi in parts:
                kind = TIMBRE.get(tim, "?")
                if pos == 0.0 and kind == "?":
                    kind = "open"
                events.append(Event(path.stem, sheet_name, len(events), deg, rng, st, pos, kind, onset, multi))
        out.extend(events)
    return out
