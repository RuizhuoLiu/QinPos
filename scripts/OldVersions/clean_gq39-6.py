...
def _(mo):
    ...
    5. **Alternate gong reading within one piece (`status="alt_gong"`)**: some pieces' score explicitly states a tuning-to-degree mapping that differs from the piece's dominant K for a handful of notes (e.g. 憶故人's header states "正調定弦 1 2 4 5 6 1 2": gong on string 1, while most of the piece's notes fit better with gong on string 3). For any note that fails the piece's primary K, we test the integer K implied by that note's own residual; if that alternate K resolves it within tolerance, we keep the note (it's real signal, not noise) but tag it `status="alt_gong"` with `K_used` recorded, distinct from the piece's primary `K`, so it's usable for training while remaining visible for a sanity check against the score.
       ...
       `gong_string_match` records whether `K_used` actually corresponds to some string's open pitch class (mod an octave). When it does, this is a genuine alternate gong-string reading. When it doesn't, the note's residual happened to resolve numerically under some integer K, but that K isn't a real retuning. It is more likely to reflect a variant reading of that specific scale degree (a 偏音 like 変徵/変宮 taking a different semitone value here than `DEGREE_SEMITONES` encodes), which is a different phenomenon that happens to be numerically indistinguishable from a gong shift for a single note.
    ...
    7. Anything still off by >0.6 semitones (under both the primary and any alternate K) is flagged `status="outlier"` and excluded from the clean set (candidates: chuo/zhu ornament pitches annotated at slide targets, genuine typos, or an unmarked mid-piece 借调 not reflected).
...
def _(OPEN_STRING_SEMITONES):
    def gong_strings_for_K(K: int) -> list[int]:
        ...
        K corresponds to a genuine alternate gong-string reading only
        if it matches some string's open pitch class modulo an octave
        (the note's `range` absorbs whichever octave). If no string
        matches, the K that resolves this note's residual isn't a real
        retuning — it's more likely that this specific scale degree
        uses a different semitone value here than DEGREE_SEMITONES
        encodes (e.g. a 変徵/変宮-style 偏音 reading), which happens to
        be numerically indistinguishable from a gong shift for a
        single note.
        """
...
def _(
    ...
    for piece, pairs in sorted(by_piece.items()):
        # pick the range convention and gong offset that explain this piece's majority of notes best
        ...
        for _e, phys in pairs:
            ...
            K_used = K
            alt_gong = False
            gong_match = gong_strings_for_K(K)
            ...
            if abs(resid) <= RESIDUAL_TOL:
                ...
                # not explained by the piece's primary K/convention, test whether a single alternate integer K (a different gong-string reading used for just this note) resolves it, before giving up as an outlier.
                alt_K = round(resid + K)
                ...
                if alt_K != K and abs(alt_resid) <= RESIDUAL_TOL:
                    ...
                    status = "alt_gong"
                    alt_gong = True
                    gong_match = gong_strings_for_K(alt_K)
                else:
                    status = "outlier"
                    gong_match = []
...
def _(mo):
    mo.md(r"""
    ## Alt_gong notes (alternate gong reading within a piece)

    Kept and usable for training, but worth a quick sanity check: does `K_used` correspond to a plausible gong-string reading (compare
    against `OPEN_STRING_SEMITONES[s]` for some strings), and does the score itself (header tuning notation, or a mode change) support
...
def _(rows):
    def print_alt_gong_rows(all_rows):
        ...
        confirmed = [r for r in alt if r["gong_string_match"]]
        unmatched = [r for r in alt if not r["gong_string_match"]]

        print(f"{len(alt)} alt_gong notes total "
              f"({len(confirmed)} match a real open string, "
              f"{len(unmatched)} don't)\n")

        print("--- confirmed: K_used matches an actual open-string pitch class ---")
        for r in sorted(confirmed, key=lambda r: (r["piece"], r["idx"])):
        ...
        print("\n--- unmatched: K_used isn't any string's open pitch — likely a "
              "variant-degree (偏音) reading at this specific note, not a real "
              "gong-string change; numerically resolved but worth a closer look ---")
        for r in sorted(unmatched, key=lambda r: (r["piece"], r["idx"])):
            print(f"  {r['piece']:20s} idx={r['idx']:4d} string={r['string']} "
                  f"position={r['position']:5.2f} degree={r['degree']} "
                  f"range={r['range_annotated']} K={r['K']:+d} -> "
...
def _(mo):
    mo.md(r"""
    ## Suspect notes (loader-inferred position split)

    These passed the residual check (mostly `status="clean"`), but their `position` was reconstructed by the loader from a packed cell rather than read directly. Worth a first-pass spot check against the score, since the split relies on a physical
    argument (same string, different degree implies different position) rather than an explicit comma in the source file.
    """)
    return
...
def _(mo):
    ...
    These are excluded from the clean set. Worth checking against the original score for chuo/zhu slide ornaments (annotated at the
    slide target rather than the starting pitch), genuine typos, or an unmarked mid-piece 借调 that a single per-piece K can't absorb
...
def _(mo):
    mo.md(r"""
    ## Same (piece, string, position) mapped to multiple degrees

    Re-run against the corrected loader's `stopped` list. Cases that were purely a loader parsing artifact (same string, degrees that
    are one scale-step apart, position now correctly split) should have dropped out already. What remains here is the set worth checking with a performer for genuine chuo/zhu/yin/nao ornaments.
    """)
    return
...
def _(mo):
    mo.md(r"""
    ## Residual by degree, across the cleaned set

    Uses the already-corrected `residual` column (correct K/K_used and best-fit convention baked in per piece), so a nonzero mean here
    for one degree across many pieces is a signal of a systematic physics-layer issue (e.g. `DEGREE_SEMITONES` or a just-intonation ratio in `theory.py`), not per-piece K/convention noise.
    """)
    return
...
def _(mo):
    mo.md(r"""
    ## Per-piece residual by degree, and by section

    Change `TARGET_PIECE` to inspect a different piece. The by-section breakdown matters for pieces like yi01/03/04/05/06 (借调, not altered tuning, so still in scope). If the outlier rows above cluster in one `section`, that section likely needs its own K rather than sharing the whole piece's single fitted K.
    """)
    return
