...
def _(mo):
    ...
    4. **Loader-level ambiguity (`suspect`)**: some cells pack multiple notes as a comma-separated list, but Excel silently turned that comma into a decimal point on integer columns (`degree`, `range`, `string`), and/or the `position` cell arrived as a single fractional value even though the row holds >1 note. `dataset_gq39.py` reconstructs these, and when it has to *infer* a split for `position` from a physical necessity argument (two different degrees on the same string can't share a position) rather than reading an explicit comma, it marks that note `suspect=True`. These are distinct from `outlier`: they may pass the residual tolerance and get `status="clean"`, but the split was inferred, not read directly, so they're worth a first-pass spot check.

    5. After convention selection, remaining exact ±1-octave residuals are isolated range typos: repaired by adjusting range, and marked `status="repaired"` so downstream users can exclude them if wanted.

    6. Anything still off by >0.6 semitones is flagged `status="outlier"` and excluded from the clean set (candidates: chuo/zhu ornament pitches annotated at slide targets, genuine typos, or an unmarked mid-piece 借调 not reflected in `section` — future work).
