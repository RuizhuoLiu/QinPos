...
def _(mo, seqs):
    ...
    open_bias = mo.ui.slider(
        ...
        label="散音 bias (right = more open strings = easier)",
    )
    harm_bias = mo.ui.slider(
        -2.0, 2.0, step=0.25, value=0.0,
        label="泛音 bias (right = more harmonics)",
    )
    cjk_labels = mo.ui.checkbox(
        value=True, label="Chinese labels (uncheck for export without CJK fonts)"
    )
    show_baseline = mo.ui.checkbox(
        value=True, label="show the bias-0 reference"
    )
    mo.vstack([
        mo.hstack([piece_pick, open_bias, harm_bias], justify="start", gap=2),
        mo.hstack([cjk_labels, show_baseline], justify="start", gap=2),
    ])
    return cjk_labels, harm_bias, open_bias, piece_pick, show_baseline
...
def _(
    ...
    if show_baseline.value and biased:
        ...
        baselines = [max(m, key=lambda c: m[c]) for m in base_margs]
...
def _(
    ...
    caption = (
        ...
        + ("" if baseline is None or baseline == argmax
...
@app.cell(hide_code=True)
def _(changed, cjk_labels, experts, margs, mo, note_slider, seq, viz):
    ...
    if changed is not None:
        _rows.append(("moved since bias 0", f"{sum(changed)} / {len(changed)}"))
