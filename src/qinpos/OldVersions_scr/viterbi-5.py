...
[VERSION 4]
    ...
    v3 (design_history/viterbi_v3_center_pull.py, 49.7% hand-crafted):
       two-sided linear pull toward CENTER_HUI = 8.5. Phase 3 learning
       on top of v3 reached 43.6% exact under the free-timbre protocol
       -- BELOW a context-free pitch lookup table (45.5%). Ablation
       showed the arc features were the cause: retraining with them
       removed scored 48.0%.
    v4 (this file): two changes aimed at that result.

       (a) HAND POSITION IS NOW CARRIED THROUGH non-stopped notes.
           v3 computed hand_travel only when BOTH endpoints of an arc
           were stopped, so 按5弦10徽 -> 散7弦 -> 按4弦3徽 cost nothing
           for a seven-hui move: the left hand teleported for free
           across every open string, and open notes are 29% of GQ39.
           Consecutive stopped notes were charged, isolated moves were
           not, so the penalty pointed in an incoherent direction. The
           DP state is now (candidate, hand), where hand is the
           position of the most recent stopped note at or before the
           current one.

       (b) STRING x HUI-BAND CROSSED FEATURES. The v3 per-string
           biases are global one-hots, so they can only encode "which
           string is used often", not "which string suits this
           register" -- which is why a per-pitch lookup table beat the
           model. Crossing string with a coarse hui band gives the node
           model register-dependent string preference, which is the
           thing the lookup table had and v3 did not.
...
through _next_hand(); do not inline that logic anywhere else.
...
#                   where hand is the last stopped position at or
#                   before the previous note (see v4 note (a))
...
def arc_features(a: Candidate, b: Candidate, hand=_UNSET) -> dict[str, float]:
    ...
    `hand` is the position of the most recent stopped note at or before
    `a`, or None if there has not been one. Omitting it reproduces the
    v3 behaviour (hand travel charged only between two stopped notes),
    which is kept only so old design_history scripts still run -- live
...
def decode_lattice(
    ...
    candidate, because hand_travel depends on where the left hand last
    landed, which can be several notes back when open or harmonic notes
    intervene. Stopped candidates collapse every incoming hand to their
    own position, so the state count only grows across runs of
    non-stopped notes.
