...
class Weights:
    ...
    is_harmonic: float = 0.0  # neutral unless the score demands
    cramped_high: float = 0.8  # the only positional force, one-sided
    string_cross: float = 0.7
    hand_travel: float = 1.0  # heavy arc weights - these dominated when no positional anchor
...
# Decoder
def decode_lattice(lattice: list[list[Candidate]], w: Weights = Weights()) -> list[Candidate]:
...
def decode(notes: list[Note], w: Weights = Weights(), kinds: list[str] | None = None) -> list[Candidate]:
    """Decode a melody. If `kinds` is given ('open'/'stopped'/
    'harmonic'), the lattice is restricted to candidates of that kind for that note."""
    lattice = []
