...
class Weights:
    ...
    hand_travel: float = 0.5
    reposition: float = 0.4  # per-hui left-hand slide cost landing the hand from open/harmonic
...
def decode(notes: list[Note], w: Weights = Weights(), kinds: list[str] | None = None) -> list[Candidate]:
    """Decode a melody. If `kinds` is given (one of 'open'/'stopped'/'harmonic',
