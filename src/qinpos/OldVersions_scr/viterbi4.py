...
[VERSION 3]
    ...
    v2 (design_history/viterbi_v2_comfort_band.py, 23.2%): flat comfort
       ...
       decisions happen.
    v3 (this file, 49.7% default / 51.5% coarse grid): two-sided linear
...
# Per-string bias features (string_1 .. string_7): a one-hot marker of
# which string a candidate uses. Hand-crafted weights leave these at 0
# (no effect); Path B learning uses them to absorb per-string timbral
# preference that position features can't express.
NODE_FEATURES = ("is_open", "is_harmonic", "below_center", "above_center",
...
class Weights:
    ...
    # per-string biases: neutral by hand, learnable by Path B
    string_1: float = 0.0
    string_2: float = 0.0
    string_3: float = 0.0
    string_4: float = 0.0
