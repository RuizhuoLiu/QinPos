...
"""Forward-backward (candidate, hand) expansion."""
    ...
    for i in range(n):
        for (j, hand), a, b in zip(states[i], alphas[i], betas[i]):
            ...
            if p <= 0.0:
                continue
            for k, v in node_features(cols[i][j], ctx[i]).items():
                if v:
                    exp_f[k] += p * v
    ...
    for i in range(1, n):
        ...
        for (k, hand), pa in zip(states[i - 1], alphas[i - 1]):
            ...
            for j, cur in enumerate(cols[i]):
                ...
                p = math.exp(pa - ac - node_cost(cur, w, ctx[i]) + betas[i][t] - logZ)
                if p <= 0.0:
                    continue
...
def train_crf(
    ...
    w = WeightVector(init)
    live = [k for k in FEATURES if k not in frozen]
    for k in frozen:
        w[k] = 0.0
    g2 = {k: 1e-8 for k in live}  # accumulated squared gradients
...
def note_marginals(notes: list[Note], w, kinds=None) -> list[dict[Candidate, float]]:
    ...
    for j, c in enumerate(cols[0]):
        key = (j, _next_hand(None, c))
        first[key] = _logsumexp([first.get(key, float("-inf")), -node_cost(c, w, ctx[0])])
    states.append(list(first.keys()))
    alphas.append([first[k] for k in states[0]])
    for i in range(1, n):
        nxt: dict[tuple[int, float | None], float] = {}
