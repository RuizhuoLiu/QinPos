...
which is exactly what the v3 results showed (43.6% vs 45.5%).
...
"how far could any pitch-only mapping possibly go on this data".

Ablation
--------
train(..., frozen=(...)) holds named weights at zero throughout
training. Because the model is linear, a weight pinned at zero is
equivalent to the feature not existing, so this is a TRUE ablation --
the remaining weights get to re-fit without it. Zeroing weights after
training instead measures something weaker: the others already
compensated for the ablated feature, so the observed drop is only an
upper bound on its real contribution.
"""
...
class WeightVector(dict):
    ...
    def zeroed(self, *feature_names: str) -> "WeightVector":
        """Copy with the named weights set to zero. POST-HOC knockout,
        not a true ablation: the surviving weights were learned with
        these present and already compensate for them, so the accuracy
        drop overstates the removed features' contribution. Use
        train(frozen=...) for the real thing."""
        out = self.copy()
...
def build_sequences(data_dir: Path, clean_csv: Path) -> list[PieceSequence]:
    ...
    Ordering: verified Aug 2026 that every GQ39 piece holds a single
...
def check_idx(data_dir: Path) -> None:
    ...
    the ones where a flat idx sort could interleave sections.
    """
...
def reach_report(seqs: list[PieceSequence], scored_only: bool = True) -> None:
    ...
    and candidates_for() share the same tuning theory, so a note passing
    one passes the other. The informative run is scored_only=False, where
    unreachable notes act as an independent check on needs_review.
    """
...
def train(
    ...
    frozen: feature names held at 0.0 for the whole run. In a linear
    model a weight pinned at zero is equivalent to the feature not
    existing, and the remaining weights re-fit around its absence, so
    this is a TRUE ablation. Compare against WeightVector.zeroed(),
    which knocks weights out after training and therefore only bounds
    a feature's contribution from above.

    use_gold_path=False reproduces the pre-reachability behaviour (raw
    expert annotation as the target). With the current data every scored
    note is reachable, so the two coincide.
    """
...
def evaluate(seqs: list[PieceSequence], w, kinds_known: bool = False) -> dict[str, float]:
    ...
    kind_fallback counts notes where the requested timbre had no
    candidate and the constraint was silently dropped. A non-zero value
    means the "timbre given" number is not a pure protocol.
    """
