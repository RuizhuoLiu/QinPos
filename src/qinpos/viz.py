...
def svg_to_jpg_bytes(
    ...
    try:
        ...
        raise ExportError("JPEG export needs Pillow: uv add pillow") from exc

    png = Image.open(BytesIO(svg_to_png_bytes(svg, scale))).convert("RGBA")
    flat = Image.new("RGB", png.size, background)
    flat.paste(png, mask=png.split()[3])
    buf = BytesIO()
    flat.save(buf, "JPEG", quality=quality, optimize=True, progressive=True)
    return buf.getvalue()


def export_raster(svg: str, path: str | Path, scale: float = 2.0, **kwargs) -> Path:
    """Write a PNG or JPEG, chosen by the path's suffix."""
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    if path.suffix.lower() in {".jpg", ".jpeg"}:
        path.write_bytes(svg_to_jpg_bytes(svg, scale, **kwargs))
    else:
        path.write_bytes(svg_to_png_bytes(svg, scale))
    return path
